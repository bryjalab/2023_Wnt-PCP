---
title: "01: proteinGroups.txt file analysis"
output: 
  html_document:
    toc: true
    toc_float: true
...
Raw spectra were processed using the MaxQuant software, and the output, **proteinGroups.txt** file is used for further post-processing using the DEP R package.
...
data.filtered <- data_unique %>%
  left_join(., BirA.sticky %>% select(GENE, AVE_SC, NUM_EXPT), by = c("name" = "GENE")) %>%
  filter(is.na(AVE_SC))
```

## Generate a SummarizedExperiment object
Create **experimental design** and filter out only the columns containing protein intensities.
...
## SD vs mean plots
Plot standard deviations vs mean plots
```{r sd-check, message=FALSE, warning=FALSE}
...
# rather setting the seed; 42 is just purely the Hitchhiker's Guide to Galaxy reference :)
set.seed(42)
